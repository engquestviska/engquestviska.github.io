...
(function () {
  var NAV_ITEMS = [
    ...
    { page: 'vocabulary', href: 'vocabulary.html', icon: 'book-a', name: 'Vocabulary' },
  ...
  function shellMarkup() {
    return [
      ...
      '  <div class="sidebar-brand-row" style="display:flex;align-items:center;margin:0 8px 25px;">',
      '    <a class="sidebar-brand" href="index.html" style="margin:0;flex:1;min-width:0;"><span class="brand-mark">' + icon('graduation-cap') + '</span><span>English Quest</span></a>',
      '    <button type="button" class="sidebar-collapse-btn" onclick="toggleSidebarCollapse()" aria-label="Collapse sidebar" title="Collapse sidebar">' + icon('panel-left-close') + '</button>',
      '  </div>',
  ...
  var COLLAPSE_KEY = 'eqTeacherSidebarCollapsed';

  window.toggleSidebarCollapse = function () {
    var collapsed = !document.body.classList.contains('sidebar-collapsed');
    setSidebarCollapsed(collapsed);
    try { window.localStorage.setItem(COLLAPSE_KEY, collapsed ? '1' : '0'); } catch (error) {}
  };
  ...
  function applySavedCollapseState() {
    var saved;
    try { saved = window.localStorage.getItem(COLLAPSE_KEY); } catch (error) { saved = null; }
